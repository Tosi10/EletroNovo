...
/**
 * Faz upload da logo da empresa para o Firebase Storage.
 * Esta função deve ser chamada uma vez para configurar a logo no storage.
 * @returns {string} URL de download da logo.
 */
export const uploadCompanyLogo = async () => {
  try {
    // No React Native, precisamos usar Asset.fromModule para imagens locais
    const { Asset } = require('expo-asset');
    
    // Carrega a imagem como asset
    const asset = Asset.fromModule(require('../assets/images/cardio2.png'));
    ...
    console.log('Firebase Storage: Iniciando upload da logo...');
    
    // Faz upload para o storage
    const storageRef = ref(storage, 'images/cardio2.png');
    const uploadTask = await uploadBytes(storageRef, blob);
    const downloadURL = await getDownloadURL(uploadTask.ref);

    console.log('Firebase Storage: Logo da empresa enviada com sucesso. URL:', downloadURL);
    return downloadURL;
  } catch (error) {
    console.error('Firebase Storage: Erro ao fazer upload da logo:', error);
...
/**
 * Busca ECGs laudados que foram solicitados para revisão (reviewRequested: true)
 * @returns {Array<object>} Lista de ECGs para revisão
 */
export const getEcgsForReview = async () => {
  try {
    const q = query(collection(db, 'ecgs'), where('status', '==', 'lauded'), where('reviewRequested', '==', true), orderBy('createdAt', 'desc'));
    const querySnapshot = await getDocs(q);
    const ecgs = [];
    for (const docSnapshot of querySnapshot.docs) {
      const ecgData = { id: docSnapshot.id, ...docSnapshot.data() };
    ...
    console.error('Erro ao buscar ECGs para revisão:', error);
    throw new Error(error.message);
  }
};
...
export const updateEcgLaudation = async (ecgId, laudationContent, laudationDoctorId, laudationDetails) => {
  try {
    const ecgDocRef = doc(db, 'ecgs', ecgId);
    // Permitir status customizado (draft ou lauded)
    let statusToSet = 'lauded';
    if (laudationDetails && laudationDetails.status) {
      statusToSet = laudationDetails.status;
    }
    await updateDoc(ecgDocRef, {
      ...
      laudedAt: statusToSet === 'lauded' ? serverTimestamp() : null,
    });
...
/**
 * Marca um ECG como solicitado para revisão (reviewRequested: true)
 * @param {string} ecgId O ID do ECG
 */
export const requestEcgReview = async (ecgId) => {
  const ecgRef = doc(db, 'ecgs', ecgId);
  await updateDoc(ecgRef, { reviewRequested: true });
};
...
export const sendPasswordReset = async (email) => {
  try {
    ...
    console.log('Firebase: Email de reset de senha enviado para:', email);
    return { success: true, message: 'Email de recuperação enviado com sucesso!' };
  } catch (error) {
    console.error('Firebase: Erro ao enviar email de reset:', error);
    
    let errorMessage = 'Ocorreu um erro ao enviar o email de recuperação.';
    if (error.code === 'auth/user-not-found') {
      errorMessage = 'Não existe uma conta com este endereço de email.';
