...
import { getPendingEcgs } from '../lib/firebase';

const EcgPendingList = () => {
  ...
  const fetchPendingEcgs = async () => {
    if (!user?.uid || user?.role !== 'medico') return;
    
    setLoading(true);
    try {
      const data = await getPendingEcgs();
      setEcgs(data);
    } catch (error) {
      Alert.alert('Erro', 'Não foi possível carregar os ECGs pendentes.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getPriorityColor = (priority) => {
    return priority === 'Urgente' ? 'text-red-400' : 'text-blue-400';
  };
  ...
  if (loading) {
    return (
      <SafeAreaView className="flex-1 justify-center items-center bg-primary">
        ...
        <Text className="text-white text-lg mt-4">Carregando ECGs pendentes...</Text>
      </SafeAreaView>
    );
  }
            ...
            <View className="flex-row items-center justify-between">
              <View className="flex-1">
                ...
                <Text className="text-gray-300 text-sm">
                  {item.age} anos | {item.sex}
                </Text>
                <Text className={`text-sm font-pmedium mt-1 ${getPriorityColor(item.priority)}`}>
                  {item.priority}
                </Text>
        ...
        ListEmptyComponent={() => (
          <View className="flex-1 justify-center items-center px-4">
            <Text className="text-white text-lg text-center mb-2">
              Nenhum ECG pendente encontrado
            </Text>
            <Text className="text-gray-300 text-center">
              Não há eletrocardiogramas aguardando laudação.
            </Text>
          </View>
        )}
        refreshControl={
